// Adapted from https://github.com/amosblomqvist/pi-config/blob/main/extensions/video-extract/index.ts
...
function mapFfmpegError(err: unknown): string {
	...
	if (code === "ENOENT")
		return "ffmpeg is not installed or not on PATH. This dotfiles flake includes ffmpeg; rebuild/switch to make it available.";
...
function mapYtDlpError(err: unknown): string {
	const { code, stderr, message } = readExecError(err);
	if (code === "ENOENT")
		return "yt-dlp is not installed or not on PATH. This dotfiles flake includes yt-dlp; rebuild/switch to make it available.";
